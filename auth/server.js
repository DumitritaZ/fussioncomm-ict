...
function hasRealAdmin() {
  try {
    return loadUsers().some(u => u.role === 'admin' && (u.status || 'active') !== 'blocked');
  } catch {
    return false;
  }
}
...
app.post('/api/login', (req, res) => {
  ...
  if (u) {
    ...
    if (!verifyPassword(u.pass, password)) {
  ...
  if (!hasRealAdmin() &&
      ident === String(ADMIN_USER).toLowerCase() &&
      password === ADMIN_PASS) {
...
app.post('/api/change-password', requireAuth, (req, res) => {
  ...
  if (req.user.id === 'admin') {
    return res.status(403).json({ ok:false, error:'default_admin_cannot_change_here' });
  }
  ...
  if (!verifyPassword(u.pass, current)) {
...
app.get('/api/users', requireAdmin, (req,res)=>{
  ...
  if (f === 'active' || f === 'blocked')
    list = list.filter(u => (u.status || 'active') === f);
